...
bool ESP8266_BuildAP ( char * pSSID, char * pPassWord, char * enunPsdMode )
{
	...
	return ESP8266_Cmd ( cCmd, "OK", 0, 3000 );
...
//	ESP8266_BuildAP ( pStrDelimiter [0], pStrDelimiter [2], pStrDelimiter [1] );
	ESP8266_BuildAP ( WiFi_name, WiFi_passWord, "2" );
	ESP8266_Cmd ( "AT+RST", "OK", "ready", 2500 ); //*
...
//	ESP8266_StartOrShutServer ( ENABLE, pStrDelimiter [0], pStrDelimiter [1] );
	ESP8266_StartOrShutServer ( ENABLE, WiFi_Port, "28800" );
	...
	{
		PC_Usart ( "\r\n正查询本模块IP……\r\n" );
	  ESP8266_Cmd ( "AT+CIFSR", "OK", "Link", 1000 );
		
		PC_Usart ( "\r\n请用手机连接刚才创建的WiFi，这里只连接一个手机，作为ID0，然后用手机网络调试助手以TCP Client连接刚才开启的服务器（AP IP）……\r\n" );
		delay_ms ( 20000 ) ;
	}	while ( ! ESP8266_Cmd ( "AT+CIPSTATUS", "+CIPSTATUS:0", 0, 1000 ) );
	...
	//	scanf ( "%s", cStrInput );
	pStr  = "客户端已经连接";
	memcpy(cStrInput, pStr, strlen(pStr)+1);
	ul = strlen ( cStrInput );
...
void ESP8266_StaTcpClient_ApTcpServer ( void )
{
  ...
  ESP8266_Choose ( ENABLE );
	...
	//ESP8266_BuildAP ( pStrDelimiter [0], pStrDelimiter [2], pStrDelimiter [1] );
	ESP8266_BuildAP ( WiFi_name, WiFi_passWord, "2" ); // 创建指定的WIFI与密码
	ESP8266_Cmd ( "AT+RST", "OK", "ready", 2500 ); //*
  ...
  } while ( ! ESP8266_JoinAP ( WiFi_connect_name, WiFi_connect_passWord ) );  //  修改连接WIFI名称 while ( ! ESP8266_JoinAP ( pStrDelimiter [0], pStrDelimiter [1] ) );
...
//	scanf ( "%s", cStrInput );
	...
	//ESP8266_StartOrShutServer ( ENABLE, pStrDelimiter [0], pStrDelimiter [1] );
	ESP8266_StartOrShutServer ( ENABLE, WiFi_Port, "28800" );  //作为服务器时的端口号和时间
...
/*
 * 函数名：ESP8266_AT+SLEEP 
 * 描述  ：对WF-ESP8266模块进行睡眠模式控制
 * 输入  ：无
 * 返回  : 无
 * 调用  ：被外部调用
 */
void ESP8266_Deep_Sleep ( void )
{
	  ...
	  ESP8266_Cmd ( "AT+SLEEP=1", "OK",0, 200);  	//未完成

}


/*
 * 函数名：ESP8266_AT_CIPSNTPCFG 设置时域和SNTP服务器，并查询时间
 * 描述  ：对WF-ESP8266模块进行AT测试启动
 * 输入  ：无
 * 返回  : 无
 * 调用  ：被外部调用
 */
void ESP8266_SNTP ( void )
{
	
	ESP8266_Cmd ( " AT+CIPSNTPCFG=1,8,\"cn.ntp.org.cn\",\"ntp.sjtu.edu.cn\",\"us.pool.ntp.org", "OK",0, 200); 

	  ESP8266_Cmd ( "AT+CIPSNTPTIME?", "OK",0, 200);  //未完成	
